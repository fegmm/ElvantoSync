...
internal class NextcloudGroupFolderClient(HttpClient client) : INextcloudGroupFolderClient
{
    ...
    {
        var request = await client.PostAsJsonAsync("/apps/groupfolders/folders", new { mountpoint = name }, cancellationToken);
        var response = await request.EnsureSuccessStatusCode()
    ...
    {
        var request = await client.DeleteAsync($"/apps/groupfolders/folders/{id}", cancellationToken);
    ...
    public async Task SetMountpoint(int id, string mountpoint, CancellationToken cancellationToken = default)
    {
        ...
        var request = await client.PostAsJsonAsync($"/apps/groupfolders/folders/{id}/mountpoint", reqBody, cancellationToken);
        request.EnsureSuccessStatusCode();
    }
    ...
    public async Task SetAcl(int id, bool enable, CancellationToken cancellationToken = default)
    {
        ...
        var request = await client.PostAsJsonAsync($"/apps/groupfolders/folders/{id}/acl", reqBody, cancellationToken);
        request.EnsureSuccessStatusCode();
    }
    ...
    public async Task AddAclManager(int id, string groupId, CancellationToken cancellationToken = default)
    {
        ...
        var request = await client.PostAsJsonAsync($"/apps/groupfolders/folders/{id}/manageACL", reqBody, cancellationToken);
        request.EnsureSuccessStatusCode();
    }
    ...
    public async Task SetPermission(int id, string groupId, Permissions permission)
    {
        ...
        var request = await client.PostAsJsonAsync($"/apps/groupfolders/folders/{id}/groups/{groupId}", reqBody);
        request.EnsureSuccessStatusCode();
    }
}
